...
.pdf-brand-row svg { flex-shrink: 0; }
...
/* ── Content — github-markdown-css inspired ── */
.markdown-body {
  font-size: 15px;
  line-height: 1.75;
  color: #111111;
...
.markdown-body h1 { font-size: 26px; padding-bottom: 8px; border-bottom: 1px solid #d1d5db; }
.markdown-body h2 { font-size: 22px; padding-bottom: 6px; border-bottom: 1px solid #e5e7eb; }
.markdown-body h3 { font-size: 18px; }
.markdown-body h4 { font-size: 16px; }
.markdown-body h5 { font-size: 15px; }
.markdown-body h6 { font-size: 14px; color: #555555; }
...
.markdown-body strong { font-weight: 700; color: #111111; }
...
.markdown-body tbody { display: table-row-group; }
.markdown-body tr { display: table-row; }
.markdown-body th, .markdown-body td { display: table-cell; }
...
.pdf-footer strong { color: #111111; }
`;
...
const buildPdfHtml = (opts: {
  ...
  const bodyHtml = renderMarkdownToHtml(opts.content);
...
<div class="pdf-page">
  ...
  <div class="pdf-header">
    <div class="pdf-brand-row">
      ${LOGO_SVG}
      <div>
        <div class="pdf-brand-name">HyperFix</div>
      </div>
    </div>
    <div class="pdf-meta">Model: ${escapeHtml(opts.modelName)} • Date: ${escapeHtml(opts.date)}</div>
  </div>
  ...
  <div class="pdf-footer">
    <span><strong>HyperFix</strong> — La fixation, notre raison d\u2019\u00eatre.</span>
...
export const generatePdfFromMarkdown = async (options: PdfExportOptions): Promise<void> => {
  ...
  const pdfPage = iframeDoc.querySelector('.pdf-page');
  if (!pdfPage) {
    document.body.removeChild(container);
    throw new Error('Cannot find .pdf-page element');
  }
