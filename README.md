...
![HyperFix](/app/opengraph-image.png)

HyperFix, la fixation — notre raison d’être.

🔗 **[Try HyperFix at hypeer.vercel.app](https://hypeer.vercel.app/)**
...
## Set HyperFix as your default search engine
...
4. **Set the search engine name**:
   - Enter `HyperFix` in the "Search engine" field.

5. **Set the search engine URL**:
   - Enter `https://hypeer.vercel.app/?q=%s` in the "URL with %s in place of query" field.
...
After completing these steps, you should be able to use HyperFix as your default search engine in Chrome.
...
2. Build the Docker image:
   ```bash
   docker build -t hypeer.vercel.app .
   ```
3. Run the container:
   ```bash
   docker run --env-file .env -p 3000:3000 hypeer.vercel.app
   ```
